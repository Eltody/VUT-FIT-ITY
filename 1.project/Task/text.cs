...
- Pokud jste tak ještě neučinili, přečtěte si důkladně pokyny k projektu a soubor s detailní
  specifikací projektu!
- Pomlčky a spojovníky jsou v tomto textu reprezentovány pouze znakem -.
  Ve vysázeném dokumentu musí být správná šířka pomlček, spojovníků a (!!) mezer podle kontextu.
- V tomto textu jsou použity tyto "dvojité uvozovky".
  Ve vysázeném dokumentu musí být uvozovky podle vzorového PDF (když nepoužijete UTF-8, řešte
  české uvozovky pomocí předdefinovaného příkazu).
- Adresy (email, http) a odkazy na číslované sekce (Sekce 2 a 3) nebo poznámky pod čarou musí
  být ve výsledném PDF vysázeny speciáními příkazy k tomu určenými.
- Nezapomeňte ve svém dokumentu uvést své jméno a emailovou adresu.
...
Pro systematické doplnění vlnek slouží volně šiřitelný program vlna od pana Olšáka, který si můžete v rámci projektu vyzkoušet.
